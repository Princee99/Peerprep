...
// Create new question (students only)
router.post('/', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO questions ( content, student_id) VALUES ($1, $2, $3) RETURNING *',
      [ content, userId]
    );
...
router.post('/company/:companyId', auth, async (req, res) => {
  try {
    const { companyId } = req.params;
    const { content } = req.body; // Only get content, not title
    const userId = req.user.user_id;
    const userRole = req.user.role;
    ...
    // FIXED: Remove title from INSERT statement
    const result = await pool.query(
      'INSERT INTO questions (content, student_id, company_id) VALUES ($1, $2, $3) RETURNING *',
      [content, userId, companyId] // Only 3 values for 3 columns
    );
