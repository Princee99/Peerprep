...
async function generatePasswordsFromExcel(filePath) {
    try {
        ...
        // Extract file name and directory from input path
        const inputDir = path.dirname(filePath);
        const inputFileName = path.basename(filePath, path.extname(filePath)); // Gets filename without extension
        
        // Create output file names based on input file name
        const hashedPasswordsFileName = `${inputFileName}_with_hashed_passwords.xlsx`;
        const reportFileName = `${inputFileName}_password_generation_report.json`;
         const outputFilePath = path.join(inputDir, hashedPasswordsFileName);
        const reportFilePath = path.join(inputDir, reportFileName);
