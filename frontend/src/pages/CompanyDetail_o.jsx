  // Mock reviews for frontend display
  const mockReviews = {
    aptitude: [
      {
        id: 1,
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'questions' && (
            ...
            >
              ...
              {/* Questions List */}
              <div className="space-y-6">
                {(reviews.length > 0 ? reviews : mockReviews[activeTab] || []).length === 0 ? (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="text-center py-12 bg-white/50 backdrop-blur-xl rounded-2xl border border-white/40"
                  >
                    ...
                    <h3 className="text-xl font-semibold text-slate-900 mb-2">No reviews available</h3>
                  </motion.div>
                ) : (
                  (reviews.length > 0 ? reviews : mockReviews[activeTab]).map((review, index) => (
                    <motion.div
                      key={review.id}
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4 className="font-semibold text-slate-900">{review.role}</h4>
                            <p className="text-sm text-slate-600">{review.placement_type || review.author}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="flex items-center space-x-1 mb-1">
                      ...
                      <p className="text-slate-700 mb-4">
                        {review.aptitude && (<><b>Aptitude:</b> {review.aptitude}<br /></>)}
                        {review.technical && (<><b>Technical:</b> {review.technical}<br /></>)}
                        {review.hr && (<><b>HR:</b> {review.hr}<br /></>)}
                        {review.tips && (<><b>Tips:</b> {review.tips}<br /></>)}
